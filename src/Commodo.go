...
<html>
	<head>
		...
		<style>
			...
			th {text-align: left; border-bottom: 1px solid #2c2c2c}
		</style>
	</head>
		...
		for {
			...
			for _, d := range dirs {
				...
				if d.IsDir() {
					...
					// folders += fmt.Sprintf("<img src=data:image/png;base64,"+icons["directory"]+" /> <a href=\"%s\">%s</a><br>\n", name, name)
					// folders += fmt.Sprintf()
				} else {
					...
					//
					tableItemTemplate.Execute(&files, item{Icon: icons["file"], Name: name, LastModified: d.ModTime().Format(DATEFORMAT), Size: strconv.FormatInt(d.Size(), 10)})
					// }
				}
			}
		}
